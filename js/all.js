    ...
    $('input', '.step0')
        .on('click', function () {
            ...
            if (i == 2) {
                ...
                $("input:not(:checked) + label", firstStep)
            ...
            if (i == 1) {
                if (l.val() == 'быстро') {
                    $(note)
                        .css('opacity', '1')
                        .text('Отлично, мы тоже ценим ваше время.');
                } else if (l.val() == 'дешево') {
                    $(note)
                        .css('opacity', '1')
                        .text('Что ж, этого стоило ожидать.');
                } else if (l.val() == 'качественно') {
                    $(note)
                        .css('opacity', '1')
                        .text('По-другому мы не работаем.');
                }
            }
            if (i == 2) {
                var s = "";
                ...
                if (s == "быстро дешево") {
                    $(note)
                        .css('opacity', '1')
                        .text('Отбросим все лишнее, оставив только самое необходимое.');
                } else if (s == "быстро качественно") {
                    $(note)
                        .css('opacity', '1')
                        .text('Это верный подход. Мы обожаем большие бюджеты!');
                    ...
                    $(note)
                        .css('opacity', '1')
                        .text('Вы никуда не торопитесь, и времени на проект предостаточно, верно?');
            ...
            $("input:disabled + label", firstStep)
                .on('click', function () {
                    num++;
                    if (num == 1)
                        $(note).css('opacity', '1').text('Вы можете выбрать не более 2х вариантов одновременно.');
                    if (num == 2)
                        $(note).css('opacity', '1').text('Настойчивость - важное качество Заказчика.');
                    if (num == 3)
                        $(note).css('opacity', '1').text('Вообще мы проверяли эту форму много раз, но что-то могло сломаться...');
                    if (num == 4)
                        $(note).css('opacity', '1').text('13070 человек нажали эту кнопку не менее четырех раз. Поздравляем, вы один из них.');
                    if (num == 5)
                        $(note).css('opacity', '1').text('С вами был я, спасибо за внимание.Ты кто такой?! Давай до свидания!');
                    }
                );
        });
	...
	*/
    ...
    $('.evaluate', '.form').click(function () {
        ...
        if ($(this).hasClass('btn_step0')) {
            ...
            $(this)
                .removeClass('btn_step0')
                .addClass('btn_step1');
            $('.step1', form).slideDown(500);
            $('html, body').animate({
                scrollTop: $(".step1", form)
                    .offset()
                    .top - 30
            }, 500);
            ...
            $(this)
                .removeClass('btn_step1')
                .addClass('btn_step2');
            $('.step2', form).slideDown(500);
            $('html, body').animate({
                scrollTop: $(".step2", form)
                    .offset()
                    .top - 30
            }, 500);
        ...
        function initSwiper() {
            ...
            $('.swiper-container.connected-to-bottom-swiper').each(function () {
                ...
                if ($t.closest('.testi-wrapper').find('.connected-to-top-swiper').length) {
                    swipers['swiper-' + $t.attr('id')]
                        .addCallback('SlideChangeStart', function (swiper) {
                            swipers['swiper-' + $t
                                    .closest('.testi-wrapper')
                                    .find('.connected-to-top-swiper')
                                    .attr('id')
                            ].swipeTo(swiper.activeIndex);
                        });
                }
            });
        }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status == 1) {
                ...
                get_block((name_form == 'order_form_2'
                    ? 'order-form-second'
                    : 'order-form'), function () {
                    init();
                    $("#error_form", '#' + (name_form == 'order_form'
                        ? 'order-form'
                        : 'order_form_2')).html(response.body);
                }, (name_form == 'order_form'
                    ? 'order-form'
                    : 'order_form_2'));
...
$(function () {
    ...
    $('#upload_design').fileupload({
        ...
        add: function (e, data) {
            ...
            data
                .context
                .attr('data-name', data.result.file_name);
        },
